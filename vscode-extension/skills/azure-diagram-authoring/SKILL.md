...
The `visio-azure` MCP server must be connected. If the server's tools are not
available, ask the user to add and trust it first (the workspace ships a sample
`.vscode/mcp.json`).
